...
class Prettifier:
    ...
    def gen_comment_block(self, comment: str) -> BlockStyleComment:
        ...
        return BlockStyleComment("".join(f"\n{indentation} * {i}" if i else f"\n{indentation} *" for i in lines) + f"\n{indentation} ")
    ...
    def format_wsc(self, comments: list[Comment], *, require_newline: bool) -> list[WSC]:
        ...
        for comment in comments:
            wsc_list.extend(
                [
                    WhiteSpace(f"\n{indent}"),
                    (self.gen_comment_block(comment) if isinstance(comment, BlockStyleComment) else comment),
                ]
            )
    ...
    def convert_key(self, obj: JString | Identifier | str) -> JString | Identifier:
        ...
        if self.key_quote:
            ...
            string = Identifier(obj) if IDENTIFIER_PATTERN.fullmatch(obj) else JString(obj).__post_init__(Quote.DOUBLE)
        obj = convert(obj)
    ...
    def prettify_object(self, obj: JObject) -> JObject:
        ...
        if len(obj) == 1 and not self.unfold_single:
            ...
            if not (sub_comments or (v and isinstance(v, (list, tuple, dict)))):  # is simple type
                k.__json_clear__()
                v.__json_clear__()
    ...
    def prettify(self, container: JObject | Array) -> JObject | Array:
        return self.prettify_object(container) if isinstance(container, JObject) else self.prettify_array(container)
