...
def config_impl(domain: str, **kwargs) -> Callable[[type], Type[ConfigModel]]:
    def wrapper(cls: type) -> type[ConfigModel]:
        ...
        if domain_tup in g_store.models:
            ...
            other = m_store.cls
            if cls.__module__ == other.__module__ and cls.__qualname__ == other.__qualname__:
                if m_store.instance is not None:
                    instance = m_store.instance
                    if sys.getrefcount(instance) > 3:  # parameter, local var `instance`, `m_store.instance`
...
def save(model: Union[T, Type[T]]) -> None:
    ...
    if inst is not None:
        ...
        m_store.location.path.write_text(dumps(_store.prettifier.prettify(document), endline=True), "utf-8")
    m_store.location.path.with_suffix(".schema.json").write_text(dumps(_store.files[m_store.location.path].get_schema()), "utf-8")
