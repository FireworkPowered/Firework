...
Unions = (Union, typing_extensions.Union)
...
def copy_meta(src: Any, dst: JType):
    if isinstance(dst, JContainer):
        dst.json_container_tail = getattr(src, "json_container_tail", dst.json_container_tail)
...
def _update_array(container: Array, data: list):
    for i in range(len(container)):
        ...
        if isinstance(val, (DataClass, dict)):
            ...
            val = convert(val.pattern if isinstance(val, re.Pattern) else val.isoformat())
...
class _KayakuDaciteTypeHook(dict):
    ...
    def __contains__(self, o: object) -> bool:
        if typing_extensions.get_origin(o) in Unions:
            return any(dict.__contains__(self, arg) for arg in typing_extensions.get_args(o))
    ...
    def __getitem__(self, k: Any) -> Any:
        if typing_extensions.get_origin(k) in Unions:
            ...
            def applier(arg):
                for func in [self[arg] for arg in typing_extensions.get_args(k) if dict.__contains__(self, arg)]:
                    arg = func(arg)
                return arg
