...
class _SuffixNode:
    ...
    def lookup(self, frags: list[str], index: int = 0) -> tuple[int, tuple[SourceSpec, PathSpec] | None]:
...
class _PrefixNode:
    ...
    def lookup(self, frags: list[str], index: int = 0) -> tuple[tuple[SourceSpec, PathSpec], FormattedPath] | None:
        ...
        if self.suffix:
            ...
            if spec:
                ...
                parts = src_spec.section.prefix + frags[index : -suffix_ind or None] + src_spec.section.suffix
