...
class Encoder:
    ...
    def encode_dict(self, obj: dict) -> None:
        self.fp.write("{")
        self.fp.write("".join(wsc.encode_wsc(w) for w in getattr(obj, "json_container_head", [])))
        if obj:
            last_k = next(reversed(obj))
            for k, v in obj.items():
                self.encode_pair(k, v)
                if k is not last_k or getattr(obj, "json_container_trailing_comma", False):
                    self.fp.write(",")
        self.fp.write("".join(wsc.encode_wsc(w) for w in getattr(obj, "json_container_tail", [])))
        self.fp.write("}")
    ...
    def encode_iterable(self, obj: list | tuple) -> None:
        ...
        if obj:
            ...
            for v in obj:
                self.encode(v)
                if v is not last_v or getattr(obj, "json_container_trailing_comma", False):
                    self.fp.write(",")
        self.fp.write("".join(wsc.encode_wsc(w) for w in getattr(obj, "json_container_tail", [])))
        self.fp.write("]")
