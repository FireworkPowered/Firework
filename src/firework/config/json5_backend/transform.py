...
class Transformer(BaseTransformer):
    ...
    @v_args(inline=True)
    def double_quote_string(self, string: tuple[str, list[int]] | None = None) -> JString:
    ...
    @v_args(inline=True)
    def single_quote_string(self, string: tuple[str, list[int]] | None = None) -> JString:
        value, linebreaks = string or ("", [])
        s = JString(value)
    ...
    def SIGNED_NUMBER(self, token: Token):  # noqa: N802
        if (
            ...
            and token.value not in {"NaN", "+NaN", "-NaN", "+Infinity", "-Infinity", "Infinity"}
        ):
    ...
    def _set_trail(obj: JContainer, children: list) -> None:
        ...
        obj.json_container_trailing_comma = isinstance(children[-3], Token) and children[-3].value == ","
