...
def insert_domain(domain: DomainType, cls: Type[ConfigModel]) -> None:
    ...
    for field_ in get_fields(cls):
        sub_dest: MountType = mount_dest + (field_.name,)
    ...
    write_schema_ref(file_store.schemas, mount_dest, file_store.generator.retrieve_name(cls))
...
def initialize(
    ...
    for src, path in specs.items():
        try:
            ...
            exceptions.append(e)
    if exceptions:
        raise ValueError(f"{len(exceptions)} occurred during spec initialization.", exceptions)
...
def bootstrap() -> None:
    ...
    for path, store in _store.files.items():
        ...
        path.write_text(dumps(_store.prettifier.prettify(document), endline=True), "utf-8")
    if exceptions:
