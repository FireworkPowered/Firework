...
class Suffix:
    ...
    def lookup(self, frags: Iterable[str]) -> tuple[int, tuple[SourceSpec, PathSpec] | None]:
        node = self
        bound = self.bound
        last_index = 0
...
class Prefix:
    ...
    def lookup(self, frags: Sequence[str], index: int = 0) -> tuple[tuple[SourceSpec, PathSpec], DestWithMount] | None:
        if index < len(frags) and (nxt_nd := self.nxt.get(frags[index], None)) and (lookup_res := nxt_nd.lookup(frags, index + 1)):
            return lookup_res
        if self.suffix:
            suffix_ind, spec = self.suffix.lookup(reversed(frags[index:]))
            if spec:
                src_spec, path_spec = spec
                parts = src_spec.section.prefix + list(frags[index : -suffix_ind or None]) + src_spec.section.suffix
