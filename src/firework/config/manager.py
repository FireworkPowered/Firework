...
from .json5_backend.types import JObject, Quote
...
class _KayakuCore:
    ...
    def register(self, domain: DomainIdent, cls: type[DataClass]) -> Path:
        ...
        update_schema_ref(file_store.schemas, mount, file_store.generator.retrieve_name(cls))
        return path
    ...
    def bootstrap_file(self, path: Path, store: _FileEntry) -> None:
        ...
        path.write_text(dumps(self.prettifier.prettify(document), endline=True), "utf-8")
        if exceptions:
            raise ExceptionGroup(f"{len(exceptions)} errors occurred bootstrapping {path}", exceptions)
...
JSONC_PRETTIFIER = Prettifier(trail_comma=False, key_quote=Quote.DOUBLE, string_quote=Quote.DOUBLE)
JSON5_PRETTIFIER = Prettifier(trail_comma=True, key_quote=False, string_quote=Quote.DOUBLE)
...
class ConfigManager:
    ...
    def __init__(
        ...
        if prettifier:
            ...
            self.prettifier: Prettifier = JSON5_PRETTIFIER if self.file_suffix != "jsonc" else JSONC_PRETTIFIER
        self._core = _KayakuCore(self.file_suffix, self.prettifier, self.get_schema_generator)
        exceptions = []
    ...
    def save(self, config: type[DataClass] | DataClass | DomainIdent) -> None:
        ...
        instance = config if isinstance(config, DataClass) and not isinstance(config, type) else self.get(cls_entry.cls, safe=True)
        if not instance:
        ...
        cls_entry.path.write_text(dumps(self._core.prettifier.prettify(document), endline=True), "utf-8")
        cls_entry.path.with_suffix(".schema.json").write_text(dumps(self._core.files[cls_entry.path].get_schema()), "utf-8")
