...
from .. import term
from ..exceptions import LumaArgumentError
...
class ErrorArgumentParser(argparse.ArgumentParser):
    ...
    def _parse_known_args(self, arg_strings: list[str], namespace: argparse.Namespace) -> tuple[argparse.Namespace, list[str]]:
        try:
...
class LumaFormatter(argparse.RawDescriptionHelpFormatter):
    ...
    def _format_usage(
        ...
        if prefix is None:
            prefix = "Usage: "
        result = super()._format_usage(usage, actions, groups, prefix)  # type: ignore
        if prefix:
